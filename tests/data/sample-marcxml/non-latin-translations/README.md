...
- **Non-Latin script-tagging.** Each `880`'s `$6` second component
  ...
  bibframe-to-bffi should preserve the script tag on the alternate-script literal.
...
- **Multiple `880` per record.** All five carry 3+ `880` fields; the
  highest is `2418440` at 10. bibframe-to-bffi's `880` walker must handle the
...
Same reason as `curated/` and `role-divergence/` — the marc-to-bibframe integration
