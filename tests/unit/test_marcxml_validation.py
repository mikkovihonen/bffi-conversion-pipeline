...
@pytest.mark.parametrize(
    ...
    [
        ...
        "b12345678x.xml",  # prefixed bib-id form, check 'x'
        "b11007849.xml",  # prefixed bib-id form, check '0-9'
    ],
)
def test_filename_pattern_accepts_both_forms(tmp_path: Path, name: str) -> None:
    ...
    (``<num>.xml``) and the canonical prefixed bib-id form
    (``b<num><check>.xml``). The upstream exporter that writes the
    corpus has emitted the ``b<num><check>`` form since 2026-05-12; the bare-digits form is
    accepted for pre-2026-05-12 corpus snapshots."""
    valid = tmp_path / name
    valid.write_text("<x/>")
...
def test_bib_id_from_filename_legacy_form() -> None:
    assert bib_id_from_filename(Path("/x/12345678.xml")) == "12345678"


def test_bib_id_from_filename_prefixed_form() -> None:
    """``b<num><check>.xml`` → ``b<num><check>`` returned verbatim.
    Downstream stages carry the canonical prefixed bib ID as the
    pipeline-internal ``bib_id``."""
    assert bib_id_from_filename(Path("/x/b11007849.xml")) == "b11007849"
...
def test_validate_accepts_synthetic_valid_records() -> None:
    for name in ("10000001.xml", "10000002.xml", "10000004.xml", "10000006.xml"):
        ...
        assert result.bib_id == name.split(".")[0]
...
def test_validate_exempts_music_records_from_33x_requirement(
    ...
    the HELMET corpus — that's a known cataloguing pattern, not a
