...
- `docs/roundtrip-debugging.md` — how to diagnose a field that is missing, wrong, retagged or **fabricated** in the reconstructed MARCXML. Localise the failing hop, classify the symptom, then match it against the catalogue of failure patterns (wrong FRBR axis, inverse traversal, arbitrary single-value selection, ownership collisions, marcKey-vs-structural, derived companions, gating). Read this before starting round-trip work.
